"""
    Hydra util functions for submitting jobs
"""
...
def get_batch(max_step: int, n_batch: int, path_aug: bool, n_traj: int):
    if path_aug:
        return max(2, int(n_batch / (max_step * max_step * n_traj)))
    else:
